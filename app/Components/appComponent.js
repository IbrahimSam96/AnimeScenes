...
export default function AppComponent() {
  ...
  return (
    <>
      ...
      <Canvas
        className={`col-start-1 col-span-8 row-start-1 bg-black`}
        shadows
        camera={{ position: startingPosition, fov: 60 }}
      >
        <EffectComposer>
          <Bloom
            mipmapBlur
...
export const LoadingScreeen = ({ started, setStarted, audio }) => {
  ...
  return (
    <div
      className={`menu col-start-1 col-span-8 row-start-1 transition-all duration-1000 ease-in-out opacity-70 ${
        started ? "hidden opacity-0 pointer-events-none" : "grid self-center "
      } `}
    >
      <Image
        alt="Viewer Advised Content"
...
export const Subtitles = ({ subtitle, setSubtitle, started }) => {
  ...
  const differencesInMilliseconds = [
    ...
    2500, // Delay for the fifth text
    3000, // Delay for the sixth text
    3000, // Delay for the seventh text
  ...
  return (
    <div
      className={`col-start-1 col-span-8 row-start-1 self-end justify-self-center transition-all duration-1000 ease-in-out z-[100] bg-[rgb(215,186,223,60%)] w-auto bg-opacity-20 backdrop-blur-lg rounded drop-shadow-lg p-4 px-8 mb-[100px] ${
        !started || subtitle == ""
    ...
    >
      <h2
        className={` text-xl text-white justify-self-center self-end font-sansCaption transition-all duration-1000 ease-in-out`}
      >
        {" "}
        {subtitle}{" "}
      </h2>
